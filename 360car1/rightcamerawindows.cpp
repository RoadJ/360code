...
//    finishImg = new QImage(120,720,QImage::Format_ARGB32);
    finishImg = new QImage(124,720,QImage::Format_ARGB32);
...
//        warp_dst = cv::Mat::zeros( 720, 120, src.type());
        warp_dst = cv::Mat::zeros( 720, 124, src.type());
...
//        dstTri[3] = cv::Point2f(22,135);
        dstTri[0] = cv::Point2f(124,0);
        dstTri[1] = cv::Point2f(124,720);
        dstTri[2] = cv::Point2f(0,527);
        dstTri[3] = cv::Point2f(0,155);
...
//        dstTri[3] = cv::Point2f(0,0);
        ...
        for (int i=0;i<warp_dst.rows;i++)
        {
            ...
            {
                startX[i] = 0 + (((float)124/193) * (i -527));
                endX[i] = 124;
