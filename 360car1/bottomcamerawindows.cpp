...
//    finishImg = new QImage(420,165,QImage::Format_ARGB32);
    finishImg = new QImage(388,193,QImage::Format_ARGB32);
...
//        warp_dst = cv::Mat::zeros( 165,420,src.type() );
        warp_dst = cv::Mat::zeros( 193,388,src.type() );
...
//      dstTri[3] = cv::Point2f( 322,30);
        dstTri[0] = cv::Point2f(388, 193);
        dstTri[1] = cv::Point2f(0,193 );
        dstTri[2] = cv::Point2f( 124,0 );
        dstTri[3] = cv::Point2f( 264,0);
...
//                tempTablePos = caleLoopupTablePos(0,(UTL1Array[ (array[k].y * mapx.cols) + (array[k].x) ].x),( UTL1Array[ (array[k].y * mapx.cols) + (array[k].x) ].y),1);
 //               writeLoopTablePos(i + 555,j,tempTablePos);
                tempTablePos = caleLoopupTablePos(0,(UTL1Array[ (array[k].y * mapx.cols) + (array[k].x) ].x),( UTL1Array[ (array[k].y * mapx.cols) + (array[k].x) ].y),1);
                writeLoopTablePos(i + 527,j,tempTablePos);
            }
            k++;
        }
    }
...
//                    tempTablePos = readLoopupTablePos(j+555,i);
                    tempTablePos = readLoopupTablePos(j+527,i);
